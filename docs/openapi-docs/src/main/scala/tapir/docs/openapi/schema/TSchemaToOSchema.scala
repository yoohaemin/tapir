  ...
  def apply(schema: TSchema): ReferenceOr[OSchema] = {
    schema match {
      case TSchema.SInteger(c) =>
        Right(OSchema(SchemaType.Integer).copy(minimum = c.collectFirst { case UnsafeConstraint(Constraint.Minimum(v: Int), _) => v }))
      ...
      case TSchema.SArray(el, c) =>
        Right(
          OSchema(SchemaType.Array)
            ...
            .copy(maxItems = c.collectFirst { case UnsafeConstraint(Constraint.MaxItems(v), _) => v }))
